...
- One search covers threads and meetings, ranked, and costs a few KB. It is loose on purpose: words are stemmed, any may match, misspelled and mis-transcribed words are widened. It does not match by meaning, so describe the thing in the user's likely words and add synonyms ("billing invoice charges"); file names, identifiers and error text work as written; "quotes" make a phrase. If it misses, reword and search again. Narrow with role: "user" for what the user asked for or decided in threads, or sources: ["meetings"] for what was said on calls.
...
You can hand work to a separate agent with a clean context. When the t3-code MCP server exposes t3_thread_create, giving it a prompt starts a new thread working on a model you choose; t3_model_list shows the providers and models the user has set up and can use right now, with their options and how much of each account's allowance is spent; t3_thread_wait returns the new thread's answer.
